...
def configure_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            f"NovaSystem - Automated Repository Installation Tool " f"(v{__version__})"
        ),
    ...
    parser.add_argument(
        "--version", action="version", version=f"NovaSystem {__version__}"
    )
    ...
    parser.add_argument(
        "--db-path", default=None, help="Path to the NovaSystem database file"
    )
    ...
    list_parser.add_argument(
        "--status", choices=["started", "completed", "error"], help="Filter by status"
    )
    list_parser.add_argument(
        "--output",
        "-o",
...
def install_repository(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        result = nova.process_repository(
            args.repository, mount_local=args.mount, detect_type=not args.no_detect
        )
        ...
        if args.output == "json":
            ...
            if "results" in result and result["results"]:
                ...
                for i, cmd_result in enumerate(result["results"], 1):
                    ...
                    status = "Success" if cmd_result["successful"] else "Failed"
                    print(f"   Status: {status}")
                    print(f"   Exit Code: {cmd_result['exit_code']}")
                    print(
                        f"   Execution Time: {cmd_result['execution_time']:.2f} seconds"
                    )
    ...
    try:
        ...
        if args.output == "json":
            ...
            if runs:
                ...
                for run in runs:
                    # Format date
                    date = datetime.fromisoformat(run["start_time"]).strftime(
                        "%Y-%m-%d %H:%M"
                    )
                    ...
                    success = (
                        "Yes"
                        if run.get("success")
                        else "No" if run.get("success") is False else "-"
                    )

                    line = (
                        f"{run['id']:<4} | {repo:<30} | "
                        f"{run['status']:<9} | {success:<7} | {date}"
                    )
                    print(line)
...
def show_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.output == "json":
            ...
            # Format dates
            start_time = datetime.fromisoformat(run["start_time"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            print(f"Start Time: {start_time}")

            if run.get("end_time"):
                end_time = datetime.fromisoformat(run["end_time"]).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            ...
            if commands:
                ...
                for i, cmd in enumerate(commands, 1):
                    # Format timestamp
                    timestamp = datetime.fromisoformat(cmd["timestamp"]).strftime(
                        "%H:%M:%S"
                    )
                    ...
                    if cmd.get("output"):
                        ...
                        if output_lines:
                            ...
                            if len(output_lines) > 1:
                                print(
                                    f"           (+ {len(output_lines)-1} more lines)"
                                )
